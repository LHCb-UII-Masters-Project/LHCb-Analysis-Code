...
for event in events: # loop through all events
  ...
  for pion in pions :
    for k1,k2,phi,phi_vtx in phi_candidates: 
      ...
      ds_dira[0] = dira_bpv(ds,event.Vertices,5*max_timing)
...
#     vtx_chi2.Fill( ds_vtx.chi2_distance(pv), is_signal )
      ...
      B_dira_limit[0] = 0.9
      if dira_bpv(ds,event.Vertices,5*max_timing)  < 0.9 : continue # if the cos of the angle between momenta is less than 0.9 discard
      ...
      for pion2 in Bs_good_pions:
            ...
            bs_dira[0] = dira_bpv(bs,event.Vertices,5*max_timing)
            ...
            if bs_vtx.chi2_distance(b_pv) < velo_time: 
              ...
              continue 
            B_dira_limit[0] = 0.9
            if dira_bpv(bs,event.Vertices,5*max_timing)  < 0.90 : continue
