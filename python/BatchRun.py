    ...
    if program == "BsRun":
        ...
        for min_ipChi2_4d in np.linspace(0,7, 15):
            for min_p in range(500, 4000, 250):
                ...
                wait_id.append(runThisScriptOnCondor(f"{basedir}/TrackSelectionBatch.py", f"Optimiser_{min_ipChi2_4d}_{str(os.getpid())[3:]}", subJobName=f"{min_p}", extraSetupCommands=pre_run, is_local=local, extraArgs=run_args))
                time.sleep(1)
