...
def MeanSignificanceControl(workspace_file,f_value,number_of_models=5,velo_time=50):
    ...
    control_significance = np.mean(significances)
    control_significance_err = np.std(significances)

    total_weight = 0.0
    weighted_sum = 0.0
    for significance, error in np.nditer([significances, significance_errors]):
        if error == 0:
    ...
    if total_weight == 0:
        print(significance)
        print(significance_errors)
    control_significance_alt = weighted_sum / total_weight
    control_significance_err_alt = (1.0 / total_weight) ** 0.5
...
if __name__ == "__main__":
    basedir= f"{path.dirname(path.realpath(__file__))}/../../.."
    signal_workspace_file = f"{basedir}/Outputs/EuanSignal/Velo50/Xi5Sigma/WSPACE.root"
    signal_efficiency_purity_file = f"{basedir}/Outputs/EuanSignal/Velo50/Xi5Sigma/PurityEfficiency.txt"
